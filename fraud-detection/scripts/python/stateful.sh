...
# Stateful Fraud Detection Demo (Python)
# Pattern-based fraud detection using PyFlink with state

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
PYTHON_SCRIPT="$PROJECT_ROOT/fraud-detection/src/main/python/scripts/stateful_fraud_detection_demo.py"
...
echo "Deploying Stateful Fraud Detection (Python, parallelism: $PARALLELISM)"
echo "Pattern: Small tx (<100) → Large tx (>50,000) = FRAUD"
...
flink run -d \
  ...
  -py /opt/flink/pyflink/python/scripts/stateful_fraud_detection_demo.py
"
...
echo "Output topic: fraud-alerts-stateful"
