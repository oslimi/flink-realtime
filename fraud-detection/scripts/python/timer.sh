...
# Timer-based Fraud Reporting Demo (Python)
# Aggregates fraud alerts using timer-based reporting

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
PYTHON_SCRIPT="$PROJECT_ROOT/fraud-detection/src/main/python/scripts/timer_reporting_demo.py"
...
echo "Deploying Timer-based Reporting (Python, parallelism: $PARALLELISM)"
...
flink run -d \
  ...
  -py /opt/flink/pyflink/python/scripts/timer_reporting_demo.py
"
